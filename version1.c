...
#include <stdlib.h>
#include <string.h>
#define N 100 // N est t'une constante qui represent la taille max que prend le nom du joueur
...
int main()
{
	...
	while (p != NULL)
	{
		...
		fprintf(monFichier,"%s\tnumero\t%d\ta fait\t%dmin %ds %dcs\n", p->premier.nom, p->premier.numero_dossard, p->premier.temps_total.min, p->premier.temps_total.sec, p->premier.temps_total.centSec);
		p = p->suivant;
	}
	printf("\n");
	a = abandonnes;
	...
	while (a != NULL)
	{
		...
		fprintf(monFichier, "Liste des abandonnes\n ");

		fprintf(monFichier,"%s\tnumero\t%d\n", a->premier.nom, a->premier.numero_dossard);
		a = a->suivant;
	}
